...
BUILD_ARGS="-s -w -X main.version=$(VERSION)-$(BUILD)"
...
compile: ensure-build-dir ## Compile herman
	$(GO_BINARY) build -ldflags $(BUILD_ARGS) -o $(APP_EXECUTABLE) ./main.go

install: ## Install herman
	go install -ldflags $(BUILD_ARGS)

compile-linux: ensure-build-dir ## Compile herman for linux
	GOOS=linux GOARCH=amd64 $(GO_BINARY) build -ldflags $(BUILD_ARGS) -o $(APP_EXECUTABLE) ./main.go

compile-pi: ensure-build-dir ## Compile herman for linux
	GOOS=linux GOARCH=arm GOARM=5 $(GO_BINARY) build -ldflags $(BUILD_ARGS) -o $(APP_EXECUTABLE) ./main.go
...
test: ensure-build-dir ## Run tests
	ENVIRONMENT=test $(GO_BINARY) test $(SRC_PACKAGES) -coverprofile ./out/coverage -short -v | grep -viE "start|no test files"
